...
const handleLogin =()=>{
  ...
  if(!formData.email) return setFormData((prev)=>({...prev , emailError:'email not given'}))
    else if(!formData.password) return setFormData((prev)=>({...prev , passwordError:'enter your password'}))
...
<p className="text-[12px] font-medium font-poppins text-red-400">{formData.emailError}</p>
              <h2 className="text-[16px] font-semibold font-poppins text-second dark:text-white mb-[8px] ">Email</h2>       
               <input  onChange={(e)=>setFormData((prev)=>({...prev , email:e.target.value , emailError:''}))}  className={`w-[312px] lg:w-[440px] border border-[#E5E7EB] rounded-[12px] py-[12px] pl-[16px] text-[14px] font-normal font-poppins dark:text-yellow-300 text-[#4B5563] outline-none`} type="text" placeholder="example@example.com" />
                </div>
{/* ---------------------- password */}
